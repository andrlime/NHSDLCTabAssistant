...
export const CreateJudge: FunctionComponent<{callback: Function, addALot: Function}> = ({callback, addALot}) => {
    ...
    return (<div className={styles.sublabel} style={{paddingLeft: "0rem", flexDirection: "column"}}>
        ...
        <div className={styles.createform}>
            <div style={{margin: "0.15rem"}}>Judge Name: <input value={judgeName} onChange={(e)=>
                setJudgeName(e.target.value)} type={'text'} style={{width: "100%"}}></input></div>
            <div style={{margin: "0.15rem"}}>Judge Email: <input value={judgeEmail} onChange={(e)=>
                setJudgeEmail(e.target.value)} type={'text'}
                style={{width: "100%", borderColor: !judgeEmail.match(/((\w|[-]|[.])+[@]\w+([.]\w+)+)/g) ? "red" : ""}}></input>
            </div>
        </div>
