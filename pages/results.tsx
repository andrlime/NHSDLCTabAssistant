  ...
  const navBar = (<div className={styles.navbar}>
    ...
    {!burger ? (<div>
      {toolbox.map((item, index) => (
        <Link key={item**index*13} href={item.link}><div className={styles.menuLabel} style={{backgroundColor: item.active ? "#ECC132" : "", color: item.active ? "black" : ""}}>{item.name}</div></Link>
      ))}
    </div>) : ""}
  </div>);
